...
public class AdministratorController {
    ...
    public AdministratorController(boolean userOn, Client client) {
    ...
    private void initialize() {
        ...
        administratorGUI.addBtnUsersListener(e -> {
            administratorGUI.dispose();
            AdministratorController administratorController = new AdministratorController(true, this.client);
        });
        ...
        try {
            if (message != null) {
                ...
                for (int i = 0; i < plants.length; i += 5) {
                    administratorGUI.addRowDgvList(new Object[]{plants[i], plants[i+1], plants[i+2], plants[i+3], plants[i+4]});
                }
            } else {
    ...
    private void updatePlant(){
        ...
        if (selectedPlantID != -1) {
            JTextField nameField = new JTextField(administratorGUI.getTableValueAt(1));
            JTextField speciesField = new JTextField(administratorGUI.getTableValueAt(2));
            JCheckBox carnivoreCheckbox = new JCheckBox("Carnivore", Boolean.valueOf(administratorGUI.getTableValueAt(3)));
            JComboBox<String> zoneComboBox = new JComboBox<>();
            zoneComboBox.addItem("FLOWER_GARDEN");
            zoneComboBox.addItem("HERBARIUM");
            zoneComboBox.addItem("ARBORETUM");
            zoneComboBox.addItem("WATER_FEATURES");
            zoneComboBox.addItem("NATIVE_PLANTS");
            zoneComboBox.setSelectedItem(administratorGUI.getTableValueAt(4));
    ...
    private void applyFiltersPlant() {
        ...
        String carnivoreFilter = String.valueOf(administratorGUI.isChkCarnivoreFilterSelected());
        String zoneFilter = administratorGUI.getSelectedZone();
    ...
    private void showPlantImage() {
        ...
        if (selectedPlantID != -1) {
            ...
            if (message.equals("null")) {
                administratorGUI.showMessage("Empty", "The selected plant has no image!");
            } else {
                try {
                    ...
                    JFrame imageFrame = new JFrame("Plant Image: " + administratorGUI.getTableValueAt(1));
                    ...
                    JOptionPane.showMessageDialog(administratorGUI, "Failed to load plant image from the internet.", "Image Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        } else {
    ...
    private void populateUserList() {
        ...
        try {
            if (message != null) {
                ...
                for (int i = 0; i < users.length; i += 5) {
                    administratorGUI.addRowDgvList(new Object[]{users[i], users[i+1], users[i+2], users[i+3], users[i+4]});
                }
            } else {
    ...
    private void addUser() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            client.sendMessage("addUser#" + name + "#" + password + "#" + type + "#"  + email + "#" + phone);
            String message = client.listenForMessage();
    ...
    private void updateUser() {
        ...
        if (selectedUserID != -1) {
            JTextField nameField = new JTextField(administratorGUI.getTableValueAt(1));
            JTextField passwordField = new JTextField();
            JTextField emailField = new JTextField(administratorGUI.getTableValueAt(3));
            JTextField phoneField = new JTextField(administratorGUI.getTableValueAt(4));
            JComboBox<String> typeComboBox = new JComboBox<>();
            typeComboBox.addItem("EMPLOYEE");
            typeComboBox.addItem("ADMINISTRATOR");
            typeComboBox.setSelectedItem(administratorGUI.getTableValueAt(2));
            ...
            if (result == JOptionPane.OK_OPTION) {
                ...
                client.sendMessage("updateUser#" + selectedUserID + "#" + name + "#" + password + "#" + type + "#" + email + "#" + phone);
                String message = client.listenForMessage();
    ...
    private void deleteUser() {
        ...
        if (selectedUserID != -1) {
            ...
            if (option == JOptionPane.YES_OPTION) {
                try {
                    client.sendMessage("deleteUser#" + selectedUserID);
                    String message = client.listenForMessage();
    ...
    private void applyFiltersUser() {
        ...
        client.sendMessage("filterUsers#" + nameFilter + "#" + selectedUserType);
        String message = client.listenForMessage();
        ...
        try {
            if (message != null) {
                ...
                administratorGUI.showMessage("Empty", "No users match the filters!");
            }
        } catch (Exception exception) {
            administratorGUI.showMessage("Empty", "No users match the filters!");
        }
